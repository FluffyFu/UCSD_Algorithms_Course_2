...
class Solver:
    ...
    This implementation uses a bottom-up approach, the tree is a legitimate BST if:
        - its left and right subtree are legitimate BST
        - the max value in the left substree is smaller than the parent.
        - the min value in the right substree is greater or equal to the parent.
    """
    MIN = int(-10E16)
    MAX = int(10E16)
    ...
    def __init__(self, n: int, tree: List[List[int]]) -> None:
        ...
        self._max_vals = [None for _ in range(n)]  # the max of subtree-i
        self._min_vals = [None for _ in range(n)]  # the min of subtree-i
    ...
    def _max(self, root: int) -> int:
        ...
        # use cached value if it has been computed.
        # this should come after root == -1 determination,
        # otherwise, -1 index would cause unexpected result
        # because that's just return the last element.
        if self._max_vals[root]:
...
class Solver2:
    """
    Top-down approach. As we go down the tree, we narrow down the legitimate value range of the
    substrees. If any of the subtree violates the legitimate range, we simply return False.
    """
    MIN = int(-10E16)
    MAX = int(10E16)
